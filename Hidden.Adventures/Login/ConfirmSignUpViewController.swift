        ...
        self.user?.confirmSignUp(self.code.text!, forceAliasCreation: true).continueWith {[weak self] (task: AWSTask) -> AnyObject? in
            ...
            DispatchQueue.main.async(execute: {
                if let error = task.error as NSError? {
                    ...
                    // This is a new user, send a notification to create a new profile
                    NotificationCenter.default.post(name: .DidRegisterNewUser, object: nil, userInfo: ["profile" : self?.profile])
